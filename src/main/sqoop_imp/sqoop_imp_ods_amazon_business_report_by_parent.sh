...
if [ $DISK_SPACE -gt 0 ];then
    sql="insert overwrite table ymx.ods_amazon_business_report_by_parent partition(company_code='${company_code}') 
        ...
        from (
            ...
            from(select 
                    id,user_account,seller_id,site,parent_asin,title,sessions,session_percentage,page_views,
                    page_views_percentage,buy_box_percentage,units_ordered,units_ordered_b2b,unit_session_percentage,
                    unit_session_percentage_b2b,currency,ordered_product_sales,ordered_product_sales_b2b,total_order_items,
                    total_order_items_b2b,generate_date,created_time,updated_time,ods_create_time
                ...
                select 
                    ...
                    unit_session_percentage_b2b,currency,ordered_product_sales,ordered_product_sales_b2b,total_order_items,
                    total_order_items_b2b,generate_date,created_time,updated_time,ods_create_time
                from ymx.ods_amazon_business_report_by_parent where company_code='${company_code}'
            ) t 
        ) tt
        where rn=1        
        "
